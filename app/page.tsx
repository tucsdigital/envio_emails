    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          addLog("error", `❌ Error ${response.status}: ${response.statusText}`);
          setError(`Error ${response.status}: ${response.statusText}`);
          return;
        }
        addLog("error", `❌ Error: ${errorData.error || 'Error desconocido'}`);
                ...
                <div className="mb-8 p-6 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 rounded-2xl border border-blue-200/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        min="5"
                        max="20"
                        value={tandaConfig.tamanoTanda}
                        onChange={(e) => setTandaConfig(prev => ({ 
                          ...prev, 
                          tamanoTanda: parseInt(e.target.value) || 10 
                        }))}
                        className="w-full px-3 py-2 border border-blue-300/50 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-blue-700 mb-2">
                        Pausa entre Tandas (min)
                      </label>
                      <input
                        type="number"
                        min="1"
                        max="15"
                        value={tandaConfig.pausaEntreTandas}
                        onChange={(e) => setTandaConfig(prev => ({ 
                          ...prev, 
                          pausaEntreTandas: parseInt(e.target.value) || 5 
                        }))}
                        className="w-full px-3 py-2 border border-blue-300/50 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-blue-700 mb-2">
                        Pausa entre Emails (seg)
                      </label>
                      <input
                        type="number"
                        min="1"
                        max="5"
                        value={tandaConfig.pausaEntreEmails}
                        onChange={(e) => setTandaConfig(prev => ({ 
                          ...prev, 
                          pausaEntreEmails: parseInt(e.target.value) || 2 
                        }))}
                        className="w-full px-3 py-2 border border-blue-300/50 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm"
                      />
                    </div>
                  ...
                  <div className="mt-4 text-center">
                    <p className="text-sm text-blue-600 font-medium">
                      📊 Para 50 emails: {Math.ceil(50 / tandaConfig.tamanoTanda)} tandas de {tandaConfig.tamanoTanda} emails
                    </p>
                    <p className="text-xs text-blue-500">
                      ⏱️ Tiempo estimado: ~{Math.ceil(50 / tandaConfig.tamanoTanda) * tandaConfig.pausaEntreTandas + Math.ceil(50 / tandaConfig.tamanoTanda) * 2} minutos
                    </p>
                  </div>
                </div>
