...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'template' | 'logs' | 'settings'>('dashboard');
  ...
  const enviarEmails = async () => {
    ...
    try {
      ...
      addLog('info', `📧 Total de destinatarios: ${statusData.destinatarios.total}`);
      addLog('info', `📎 PDF encontrado: ${statusData.archivos.pdfExiste ? '✅ Sí' : '❌ No'}`);
      addLog('info', `🔐 Credenciales: ${statusData.configuracion.credencialesConfiguradas ? '✅ Configuradas' : '❌ Faltantes'}`);
      ...
      if (data.success) {
        ...
        addLog('info', `📊 Resumen: ${data.estadisticas.exitosos} exitosos, ${data.estadisticas.fallidos} fallidos`);
        
        data.resultados.forEach((resultado: EmailResult) => {
          if (resultado.success) {
            addLog('success', `✅ Email enviado a: ${resultado.email}`);
          } else {
            addLog('error', `❌ Error enviando a ${resultado.email}: ${resultado.error}`);
          }
        });
  ...
  const verificarEstado = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addLog('info', `📎 PDF: ${data.archivos.pdfExiste ? '✅ Encontrado' : '❌ No encontrado'}`);
        addLog('info', `🔐 Credenciales: ${data.configuracion.credencialesConfiguradas ? '✅ Configuradas' : '❌ Faltantes'}`);
  ...
  const getLogColor = (type: LogEntry['type']) => {
    switch (type) {
      case 'success': return 'text-emerald-600 bg-emerald-50/60 border-emerald-200/40';
      case 'error': return 'text-red-600 bg-red-50/60 border-red-200/40';
      case 'warning': return 'text-amber-600 bg-amber-50/60 border-amber-200/40';
      default: return 'text-slate-600 bg-slate-50/60 border-slate-200/40';
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-slate-600 font-medium">Sistema de envío masivo 2025</p>
              </div>
            </div>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl p-8 shadow-2xl shadow-black/10 hover:shadow-3xl hover:shadow-blue-500/20 transition-all duration-500 hover:scale-105">
                  <div className="flex items-center">
                    ...
                    <div className="ml-6">
                      <p className="text-sm font-semibold text-blue-700 uppercase tracking-wide">Destinatarios</p>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl p-8 shadow-2xl shadow-black/10 hover:shadow-3xl hover:shadow-emerald-500/20 transition-all duration-500 hover:scale-105">
                  <div className="flex items-center">
                    ...
                    <div className="ml-6">
                      <p className="text-sm font-semibold text-emerald-700 uppercase tracking-wide">PDF</p>
                      <p className="text-4xl font-bold text-slate-900">
              ...
              <div className="group relative">
                ...
                <div className="relative bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl p-8 shadow-2xl shadow-black/10 hover:shadow-3xl hover:shadow-purple-500/20 transition-all duration-500 hover:scale-105">
                  <div className="flex items-center">
                    ...
                    <div className="ml-6">
                      <p className="text-sm font-semibold text-purple-700 uppercase tracking-wide">Credenciales</p>
                      <p className="text-4xl font-bold text-slate-900">
                  ...
                  <div className="mb-8">
                    ...
                    {currentEmail && (
                      <p className="text-sm text-slate-600 mt-4 text-center">
                        Enviando a: <span className="font-semibold text-blue-600">{currentEmail}</span>
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="relative">
                ...
                <div className="relative bg-white/80 backdrop-blur-xl border border-red-200/40 rounded-3xl p-8 shadow-2xl shadow-red-500/20">
                  <div className="flex items-center">
                    ...
                    <div className="ml-6">
                      <h3 className="text-2xl font-bold text-red-900">Error Detectado</h3>
                      <div className="mt-2 text-red-800 font-medium text-lg">{error}</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="relative bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl p-12 shadow-2xl shadow-black/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-200/50 rounded-3xl p-8 text-center shadow-xl shadow-slate-200/20 hover:shadow-2xl hover:shadow-slate-200/30 transition-all duration-300 hover:scale-105">
                      <div className="text-5xl font-bold text-slate-900 mb-2">{result.estadisticas.total}</div>
                      <div className="text-slate-700 font-semibold uppercase tracking-wide">Total</div>
                    </div>
                    <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border border-emerald-200/50 rounded-3xl p-8 text-center shadow-xl shadow-emerald-200/20 hover:shadow-2xl hover:shadow-emerald-200/30 transition-all duration-300 hover:scale-105">
                      <div className="text-5xl font-bold text-emerald-700 mb-2">{result.estadisticas.exitosos}</div>
                      <div className="text-emerald-700 font-semibold uppercase tracking-wide">Exitosos</div>
                    </div>
                    <div className="bg-gradient-to-br from-red-50 to-red-100/50 border border-red-200/50 rounded-3xl p-8 text-center shadow-xl shadow-red-200/20 hover:shadow-2xl hover:shadow-red-200/30 transition-all duration-300 hover:scale-105">
                      <div className="text-5xl font-bold text-red-700 mb-2">{result.estadisticas.fallidos}</div>
                      <div className="text-red-700 font-semibold uppercase tracking-wide">Fallidos</div>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setEmailTemplate(prev => ({ ...prev, signature: e.target.value }))}
                        rows={4}
                        className="w-full px-6 py-4 border border-slate-300/50 rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 text-slate-700 font-medium resize-none bg-white/50 backdrop-blur-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEmailTemplate(prev => ({ ...prev, companyName: e.target.value }))}
                        className="w-full px-6 py-4 border border-slate-300/50 rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 text-slate-700 font-medium bg-white/50 backdrop-blur-sm"
                ...
                <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-3xl p-8 border border-slate-200/50 shadow-xl">
                  <div className="bg-white rounded-2xl shadow-2xl border border-slate-200/50 overflow-hidden">
                    <div className="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 text-white p-8 text-center">
                      <h3 className="font-bold text-2xl">{emailTemplate.subject}</h3>
                    </div>
                    <div className="p-8 space-y-6">
                      <p className="font-semibold text-slate-800 text-xl">{emailTemplate.greeting}</p>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 to-slate-100/50 p-6 text-center text-sm text-slate-500 border-t border-slate-200/50">
                      © 2024 {emailTemplate.companyName}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl p-10 h-[600px] overflow-y-auto font-mono text-sm border border-slate-700/50 shadow-2xl shadow-slate-900/50">
                {logs.length === 0 ? (
                  <div className="text-slate-400 text-center py-20">
                    ...
                    <p className="text-slate-500 text-lg mt-2">Usa los botones del dashboard para comenzar</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {logs.map((log) => (
                      <div key={log.id} className={`p-5 rounded-2xl border-2 ${getLogColor(log.type)} shadow-lg backdrop-blur-sm`}>
                        <span className="text-slate-400 font-mono">[{log.timestamp}]</span>{' '}
                        <span className="mr-4 text-xl">{getLogIcon(log.type)}</span>
                        <span className="font-medium text-lg">{log.message}</span>
                      </div>
                    ))}
                    <div ref={logsEndRef} />
                  </div>
            ...
            <div className="relative bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl p-12 shadow-2xl shadow-black/10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
                <div className="space-y-6">
                  <h3 className="text-2xl font-bold text-slate-700 mb-6">📋 Archivos Requeridos</h3>
                  <ul className="space-y-4 text-slate-600">
                    <li className="flex items-center p-5 bg-gradient-to-r from-blue-50 to-blue-100/50 rounded-2xl border border-blue-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      ...
                      <code className="bg-blue-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">data/destinatarios.json</code>
                    </li>
                    <li className="flex items-center p-5 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-2xl border border-emerald-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      ...
                      <code className="bg-emerald-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">documento.pdf</code>
                    </li>
                    <li className="flex items-center p-5 bg-gradient-to-r from-purple-50 to-purple-100/50 rounded-2xl border border-purple-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      ...
                      <code className="bg-purple-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">.env.local</code>
                    </li>
                  </ul>
                </div>

                <div className="space-y-6">
                  <h3 className="text-2xl font-bold text-slate-700 mb-6">🔐 Variables de Entorno</h3>
                  <ul className="space-y-4 text-slate-600">
                    <li className="flex items-center p-5 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-2xl border border-emerald-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      <span className="w-4 h-4 bg-emerald-500 rounded-full mr-4 shadow-lg"></span>
                      <code className="bg-emerald-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">GMAIL_USER</code>
                    </li>
                    <li className="flex items-center p-5 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-2xl border border-emerald-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      <span className="w-4 h-4 bg-emerald-500 rounded-full mr-4 shadow-lg"></span>
                      <code className="bg-emerald-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">GMAIL_PASSWORD</code>
                    </li>
                    <li className="flex items-center p-5 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-2xl border border-emerald-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      <span className="w-4 h-4 bg-emerald-500 rounded-full mr-4 shadow-lg"></span>
                      <code className="bg-emerald-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">SMTP_HOST</code>
                    </li>
                    <li className="flex items-center p-5 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-2xl border border-emerald-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                      <span className="w-4 h-4 bg-emerald-500 rounded-full mr-4 shadow-lg"></span>
                      <code className="bg-emerald-100 px-4 py-2 rounded-xl font-mono text-sm font-semibold">SMTP_PORT</code>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="p-10 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 rounded-3xl border border-blue-200/50 backdrop-blur-sm">
                ...
                <ol className="space-y-4 text-blue-800 text-lg">
                  <li className="flex items-start">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 mt-0.5 shadow-lg">1</span>
                    <span><strong>Configura las variables de entorno</strong> en tu archivo <code className="bg-blue-200 px-3 py-1 rounded-lg font-mono">.env.local</code></span>
                  </li>
                  <li className="flex items-start">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 mt-0.5 shadow-lg">2</span>
                    <span><strong>Coloca tu PDF</strong> con el nombre <code className="bg-blue-200 px-3 py-1 rounded-lg font-mono">documento.pdf</code> en la raíz</span>
                  </li>
                  <li className="flex items-start">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 mt-0.5 shadow-lg">3</span>
                    <span><strong>Personaliza el template</strong> del email en la pestaña "Template"</span>
                  </li>
                  <li className="flex items-start">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 mt-0.5 shadow-lg">4</span>
                    <span><strong>Verifica el estado</strong> del sistema</span>
                  </li>
                  <li className="flex items-start">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 mt-0.5 shadow-lg">5</span>
                    <span><strong>Ejecuta el envío</strong> de emails</span>
                  </li>
                </ol>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-3xl shadow-3xl max-w-5xl w-full max-h-[90vh] overflow-hidden border border-white/40">
            <div className="flex items-center justify-between p-8 border-b border-slate-200/50">
              <h3 className="text-2xl font-bold text-slate-900">👁️ Vista Previa del Email</h3>
              <button
